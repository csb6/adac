...
void check_module(Module* module)
{
    ...
    if(!module->tables) {
        error_print2(1, "Module must have at least 1 table");
        error_exit();
    }
    ...
    switch(type_decl->kind) {
        case TYPE_RANGE:
            if(type_decl->u.range.start > type_decl->u.range.end) {
                error_print2(type_decl->line_num, "Start of range must be less than or equal to end of range");
                error_exit();
            }
            break;
        case TYPE_OPTION: {
        ...
        default:
            error_print2(type_decl->line_num, "Invalid type kind");
            error_exit();
    }
}
