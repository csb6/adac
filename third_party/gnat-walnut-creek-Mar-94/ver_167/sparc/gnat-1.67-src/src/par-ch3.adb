...
package body Ch3 is
   ...
   procedure P_Identifier_Declarations (Decls : List_Id; Done : out Boolean);
   --  Scans out a set of declarations for an identifier or list of
   --  identifiers, and appends them to the given list. Done indicates 
   --  whether or not there may be additional declarative items to scan.
   --  If Done is True, then a decision has been made that there are no
   --  more items to scan. If Done is False, then there may be additional 
   --  declarations to scan.

   procedure P_Begin_For_Missing_Body;
   --  Scans out a begin for the case of a missing subprogram body
   ...
   function P_Type_Declaration return Node_Id is
      Type_Loc         : Source_Ptr;
      Type_Start_Col   : Column_Number_Type;
      Ident_Node       : Node_Id;
      Decl_Node        : Node_Id;
      Discr_List       : List_Id;
      Unknown_Dis      : Boolean;
   ...
   begin
      ...
      if Token = Tok_Semicolon then
         ...
         Set_Has_Unknown_Discriminants (Decl_Node, Unknown_Dis);
      ...
      if Token_Name = Name_Abstract then
         Check_9X_Keyword (Tok_Abstract, Tok_Tagged);
         Check_9X_Keyword (Tok_Abstract, Tok_New);
      end if;
      ...
      if Token_Name = Name_Tagged then
         Check_9X_Keyword (Tok_Tagged, Tok_Private);
         Check_9X_Keyword (Tok_Tagged, Tok_Limited);
         Check_9X_Keyword (Tok_Tagged, Tok_Record);
      end if;
      ...
      --  Switch on token following the IS
      ...
      case Token is
         ...
         when Tok_Limited =>
            ...
            if Token = Tok_Record or else Token = Tok_Null then
               if Ada_83 then
                  Error_Msg_SP
                     ("limited record declaration not allowed in Ada 83!");
               end if;
         ...
         --  Here we have an identifier after the IS, which is certainly wrong
         --  and which might be one of several different mistakes.

         when Tok_Identifier =>

            --  First case, if identifier is on same line, then probably we
            --  have something like "type X is Integer .." and the best
            --  diagnosis is a missing NEW. Note that the missing new message
   ...
   --  Parsed by P_Type_Declaration (3.3.1)
   ...
   begin
      ...
      if No (Constr_Node) then
         ...
         Indic_Node := New_Node (N_Subtype_Indication, Token_Ptr);
   ...
   --  operator is parsed by P_Range_Or_Subtype_Mark (4.4)
   ...
   begin
      if Token = Tok_Array then
         ...
         --  are 'CLASS and 'BASE and 'STANDARD_ACCESS. Anything else is an
         --  error which we may as well diagnose here.
   ...
   --  Cases starting with identifiers are parsed by P_Identifier_Declarations.
   --  The concurrent cases are parsed by either P_Task or P_Protected.

   --------------------------------------------------------
   -- 3.2  Identifier Declarations (Also 7.4, 8.5, 11.1) --
   --------------------------------------------------------
   ...
   procedure P_Identifier_Declarations (Decls : List_Id; Done : out Boolean) is
      Decl_Node   : Node_Id;
      Type_Node   : Node_Id;
   ...
   begin
      ...
      --  really meant is an assignment statement, and that the declarations
      --  are complete (i.e. there was a missing BEGIN). In this case we reset
      --  the scan pointer to the identifier and indicate that there are no
      --  more declarations.
      ...
      --  Otherwise we have an error situation
         ...
         if Token_Name = Name_Protected then
            Check_9X_Keyword (Tok_Protected, Tok_Identifier);
            Check_9X_Keyword (Tok_Protected, Tok_Type);
            Check_9X_Keyword (Tok_Protected, Tok_Body);
      ...
      --  Come here with an identifier list and colon scanned out. We now build
      --  the nodes for the declarative items. One node is built for each
      --  identifier in the list, with the type information being repeated by
      --  rescanning the appropriate section of source.
      ...
      Ident_Loop : loop
         ...
         if Token_Name = Name_Aliased then
            Check_9X_Keyword (Tok_Aliased, Tok_Array);
            Check_9X_Keyword (Tok_Aliased, Tok_Identifier);
            Check_9X_Keyword (Tok_Constant, Tok_Identifier);
         end if;
            ...
            --  Constant object declaration
               ...
               if Token = Tok_Array then
                  Set_Object_Definition
                    (Decl_Node, P_Constrained_Array_Definition);
               else
         ...
         --  Array case
            ...
            Decl_Node := New_Node (N_Object_Declaration, Ident_Sloc);
            Set_Object_Definition (Decl_Node, P_Constrained_Array_Definition);
         ...
         if Token = Tok_Colon_Equal then
            ...
            if Nkind (Decl_Node) = N_Object_Declaration then
               ...
               Error_Msg_SP ("initialization not allowed here");
   ...
   -----------------------------
   -- 3.2  Number Declaration --
   -----------------------------

   --  Parsed by P_Identifier_Declarations(3.2)

   ------------------------------------------------------------------------
   -- 3.4, 7.3  Derived Type Definition or Private Extension Declaration --
   ------------------------------------------------------------------------

   --  DERIVED_TYPE_DEFINITION ::=
   --    new [abstract] parent_SUBTYPE_INDICATION [TYPE_EXTENSION_PART]

   --  PRIVATE_EXTENSION_DECLARATION ::=
   --     type DEFINING_IDENTIFIER [DISCRIMINANT_PART] is
   --       new [abstract] ancestor_SUBTYPE_INDICATION with PRIVATE;
   ...
   begin
      ...
      if Token_Name = Name_Abstract then
         Check_9X_Keyword (Tok_Abstract, Tok_Identifier);
      end if;

      if Token = Tok_Abstract then
         Set_Abstract_Present (Typedef_Node, True);
         Scan; -- past ABSTRACT
      end if;
      ...
      if Token = Tok_With then
         ...
         if Token = Tok_Private then
            ...
            --  Discriminant_Specifications, Has_Unknown_Discriminants,
   ...
   begin
      ...
      --  If no double dot, must be range attribute
   ...
   begin
      if Token = Tok_Char_Literal then
         ...
         Literal_Node := Token_Node;
         Change_Identifier_To_Defining_Identifier (Literal_Node);

      else
         Error_Msg_SC ("enumeration literal expected");
         raise Error_Resync;
      end if;

      Scan; -- past character literal or identifier
      return Literal_Node;
   ...
   --  Parsed by P_Enumeration_Literal_Specification (3.5.1)
   ...
   begin
      Definition_Node := New_Node (N_Floating_Point_Definition, Token_Ptr);
      Scan; -- past DIGITS
      Expr_Node := P_Expression_No_Right_Paren;
      Check_Simple_Expression_In_Ada_83 (Expr_Node);
      Set_Digits_Expression (Definition_Node, Expr_Node);
      Set_Real_Range_Specification
        (Definition_Node, P_Real_Range_Specification_Opt);
   ...
   begin
      ...
      if Token = Tok_Digits then
         if Ada_83 then
            Error_Msg_SC ("decimal fixed types not allowed in Ada 83!");
         end if;
      ...
      if Token = Tok_Range then
         Set_Real_Range_Specification
            (Def_Node, P_Real_Range_Specification_Opt);
      end if;
   ...
   --  This is normally parsed by P_Array_Type_Definition (3.6). The function
   --  here is used in situations where only a constrained array definition
   --  is acceptable (i.e. in constant and variable declarations).

   --  The caller has checked that the initial token is ARRAY

   --  Error recovery: can raise Error_Resync

   function P_Constrained_Array_Definition return Node_Id is
      Def_Node : Node_Id;

   begin
      Def_Node := P_Array_Type_Definition;

      if Nkind (Def_Node) = N_Unconstrained_Array_Definition then
         Error_Msg ("cannot declare objects of unconstrained array types",
           Sloc (Def_Node));
         return Error;
      else
         return Def_Node;
      end if;
   end P_Constrained_Array_Definition;
   ...
   begin
      ...
      --  All set if range attribute scanned
   ...
   --  No tree node is constructed for a component definition. Instead, the
   --  Subtype_Indication and Aliased_Present fields of the parent node (which
   --  is an array definition or component declaration node) are set. The
   --  processing for setting these fields is in the parent node function.
   ...
   begin
      if Token /= Tok_Left_Paren then
         ...
         if Token = Tok_Box then
            ...
            if Ada_83 then
               Error_Msg_SC ("Unknown discriminants not allowed in Ada 83");
            end if;
   ...
   begin
      if Token = Tok_Left_Paren then
         ...
         Specification_Loop : loop
            ...
            Ident_Loop : loop
               Specification_Node :=
                 New_Node (N_Discriminant_Specification, Token_Ptr);
               Set_Defining_Identifier (Specification_Node, Idents (Ident));

               if Token = Tok_Access then
                  if Ada_83 then
                     Error_Msg_SC 
                       ("access discriminants not allowed in Ada 83!");
                  end if;
               ...
               if Token = Tok_Colon_Equal then
                  Scan; -- past :=
                  Set_Expression (Specification_Node, P_Expression);
               end if;
   ...
   --  Parsed by P_Index_Or_Discriminant_Constraint (3.6.2)
   ...
   begin
      ...
      loop
         ...
         --  Check range attribute
            ...
            Append (P_Range_Attribute (Expr_Node), Constr_List);
            goto Loop_Continue;
   ...
   --  RECORD_TYPE_DEFINITION ::= [tagged] [limited] RECORD_DEFINITION

   --  There is no node in the tree for a record type definition. Instead
   --  a record definition node appears, with possible Tagged_Present and
   --  Limited_Present flags set.
      ...
      if Token = Tok_Null then
         ...
         if Token /= Tok_Case then
            Component_Scan_Loop : loop
               P_Component_Declarations (Decls_List);
               P_Pragmas_Opt (Decls_List);
         ...
         Set_Component_Declarations (Component_List_Node, Decls_List);
         return Component_List_Node;
      end if;
   ...
   procedure P_Component_Declarations (Decls : List_Id) is
      Decl_Node  : Node_Id;
      Scan_State : Saved_Scan_State;
      Num_Idents : Nat;
      Ident      : Nat;
   ...
   begin
      ...
      Ident_Loop : loop
         begin
            Decl_Node := New_Node (N_Component_Declaration, Token_Ptr);
            Set_Defining_Identifier (Decl_Node, Idents (Ident));
         ...
         if Token = Tok_Others then
            ...
            begin
               ...
               if Token = Tok_Dot_Dot then
                  ...
                  Append (P_Range_Attribute (Expr_Node), Choices);
   ...
   begin
      ...
      if Token_Name = Name_Protected then
         Check_9X_Keyword (Tok_Protected, Tok_Procedure);
         Check_9X_Keyword (Tok_Protected, Tok_Function);
      end if;
      ...
      if Token = Tok_Procedure then
         if Ada_83 then
            Error_Msg_SC ("access to procedure not allowed in Ada 83!");
         end if;

         Type_Def_Node := New_Node (N_Access_Procedure_Definition, Access_Loc);
         Scan; -- past PROCEDURE
         Set_Parameter_Specifications (Type_Def_Node, P_Formal_Part_Opt);
         Set_Protected_Present (Type_Def_Node, Prot_Flag);

      elsif Token = Tok_Function then
         if Ada_83 then
            Error_Msg_SC ("access to function not allowed in Ada 83!");
         end if;
         ...
         if Token = Tok_All or else Token = Tok_Constant then
            if Ada_83 then
               Error_Msg_SC ("access modifiers not allowed in Ada 83!");
            end if;
   ...
   begin
      ...
      loop
         P_Declarative_Items (Decls, Done);
         exit when Done;
      end loop;
   ...
   procedure P_Declarative_Items (Decls : List_Id; Done : out Boolean) is
      Scan_State : Saved_Scan_State;
   ...
   begin
      ...
      case Token is
         ...
         when Tok_Identifier =>
            P_Identifier_Declarations (Decls, Done);
         ...
         --  PRIVATE definitely terminates the declarations, and we don't
         --  check for a misuse of a reserved keyword in this case.
         ...
         when others =>
            ...
            --  Here to we check for a reserved identifier
   ...
   --  Body is parsed by P_Declarative_Item_Opt (3.11)

   -----------------------
   -- 3.11  Proper Body --
   -----------------------

   --  Proper body is parsed by P_Declarative_Item_Opt (3.11)

   ----------------------------------
   -- 3.11  BEGIN for Missing Body --
   ----------------------------------

   --  This procedure is called if a BEGIN is encountered during the scan of
   --  declarations in a declarative part, or at the outer level, and there
   --  is a subprogram declaration outstanding for which no body has been
   --  supplied. This is the case where we assume that the semicolon in the
   --  subprogram declaration should really have been is. The active SIS
   --  entry describes the subprogram declaration. On return the declaration
   --  has been modified to become a body.

   --  Error recovery: cannot raise Error_Resync

   procedure P_Begin_For_Missing_Body is
      Specification_Node : Node_Id;
      Outer_Decls        : List_Id;
      Decl_Node          : Node_Id;
      Body_Node          : Node_Id;
   ...
   begin
      ...
      --  First issue the error message. If we had a missing semicolon in the
      --  declaration, then change the message to <missing "is">

      if SIS_Missing_Semicolon_Message /= No_Error_Msg then
         Change_Error_Text     -- Replace: "missing semicolon"
           (SIS_Missing_Semicolon_Message, "missing ""is""     ");

      --  Otherwise we saved the semicolon position, so complain about it

      else
         Error_Msg ("semicolon should be IS", SIS_Semicolon_Sloc);
      end if;

      --  The next job is to fix up any declarations that occurred between
      --  the procedure header and the BEGIN. These got chained to the outer
      --  declarative region (immediately after the procedure declaration)
      --  and they should be chained to the subprogram itself, which is a body
      --  rather than a spec.
      ...
      --  Now make the scope table entry for the Begin-End and scan it out
