...
package body Endh is
   ...
   begin
      ...
      --  that we know there is a missing semicolon in any case).
   ...
   begin
      Etype := Scope.Table (Scope.Last).Etyp;
      Error_Msg_Col    := Scope.Table (Scope.Last).Ecol;
      Error_Msg_Node_1 := Scope.Table (Scope.Last).Labl;
      Error_Msg_Sloc_1 := Scope.Table (Scope.Last).Sloc;

      if Etype = E_Case then
         Error_Msg_SC ("`END CASE;` expected@ for CASE on line #!");

      elsif Etype = E_If then
         Error_Msg_SC ("`END IF;` expected@ for IF on line #!");

      elsif Etype = E_Loop then
         if Error_Msg_Node_1 = Empty then
            Error_Msg_SC
              ("`END LOOP;` expected@ for LOOP on line #!");
         else
            Error_Msg_SC ("`END LOOP&;` expected@!");
         end if;

      elsif Etype = E_Record then
         Error_Msg_SC
           ("`END RECORD;` expected@ for RECORD on line #!");

      elsif Etype = E_Select then
         Error_Msg_SC
           ("`END SELECT;` expected@ for SELECT on line #!");

      elsif Etype = E_Name then
         if Error_Msg_Node_1 = Empty then
            Error_Msg_SC ("`END;` expected@ for BEGIN on line #!");
         else
      ...
      else -- Etype = E_Suspicious_Is or E_Bad_Is
   ...
   begin
      ...
      if Etype = E_Case then
         Error_Msg_BC ("missing `END CASE;` for CASE on line #!");

      elsif Etype = E_If then
         Error_Msg_BC ("missing `END IF;` for IF on line #!");

      elsif Etype = E_Loop then
         if Error_Msg_Node_1 = Empty then
            Error_Msg_BC ("missing `END LOOP;` for LOOP on line #!");
         else
         ...
         Error_Msg_SC
           ("missing `END RECORD;` for RECORD on line #!");

      elsif Etype = E_Select then
         Error_Msg_BC
           ("missing `END SELECT;` for SELECT on line #!");

      elsif Etype = E_Name then
         if Error_Msg_Node_1 = Empty then
            Error_Msg_BC ("missing `END;` for BEGIN on line #!");
         else
   ...
   begin
      ...
      --  Otherwise we do have an END present
         ...
         for I in reverse 1 .. Scope.Last - 1 loop
            Evaluate_End_Entry (I);
