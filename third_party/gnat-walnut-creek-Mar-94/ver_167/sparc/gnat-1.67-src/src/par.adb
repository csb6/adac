...
procedure Par is
   ...
   --  get reset by occurrence of the Ada_83 or Ada_9X pragmas).
   ...
   --  Note: the actual definition of the exception is in module Exceptions.
   ...
   type Expr_Form_Type is (
      ...
      EF_Call_Name,    -- Simple expression which could also be a call or name
      EF_Call,         -- Simple expression which could also be a call
   ...
   SS_Eftm_Eltm_Sreq : constant SS_Rec := SS_Rec'(T, T, F, F, T, F, F);
   SS_Eltm           : constant SS_Rec := SS_Rec'(F, T, F, F, F, F, F);
   SS_Eltm_Ortm      : constant SS_Rec := SS_Rec'(F, T, F, T, F, F, F);
   SS_Eltm_Ortm_Tatm : constant SS_Rec := SS_Rec'(F, T, F, T, F, T, F);
   SS_Extm_Sreq      : constant SS_Rec := SS_Rec'(F, F, T, F, T, F, F);
   SS_None           : constant SS_Rec := SS_Rec'(F, F, F, F, F, F, F);
   SS_Ortm           : constant SS_Rec := SS_Rec'(F, F, F, T, F, F, F);
   SS_Ortm_Sreq      : constant SS_Rec := SS_Rec'(F, F, F, T, T, F, F);
   SS_Sreq           : constant SS_Rec := SS_Rec'(F, F, F, F, T, F, F);
   SS_Sreq_Whtm      : constant SS_Rec := SS_Rec'(F, F, F, F, T, F, T);
   SS_Whtm           : constant SS_Rec := SS_Rec'(F, F, F, F, F, F, T);
   ...
   package Scope is new Table (
      Component_Type => Scope_Table_Entry,
      Index_Type     => Int,
      Low_Bound      => 0,
      Initial        => 50,
      Increment      => 100,
   ...
   package Ch3 is
      ...
      procedure P_Component_Declarations (Decls : List_Id);
      --  Scan out one or more component declarations and append them to
      --  the given list. Only scans out more than one declaration in the
   ...
   package Ch4 is
      ...
      function P_Call_Name                            return Node_Id;
      ...
      function P_Range_Attribute (Prefix_Node : Node_Id) return Node_Id;
      --  Scan a range attribute. The caller has scanned out the prefix.
      --  The current token is known to be an apostrophe and the following
      --  token is known to be RANGE.
   end Ch4;

   package Ch5 is
      function P_Statement_Call_Name
        (Call_Name_Node : Node_Id) return Node_Id;
      --  Given a node representing a function call or name, converts it
      --  to the syntactically corresponding procedure call statement.

      procedure P_Decls_Begin_End (Parent : Node_Id);
   ...
   package Ch6 is
      ...
      function P_Formal_Part_Opt                      return List_Id;
   ...
   package Ch11 is
      function P_Exception_Handler_Part               return List_Id;
   ...
   package Util is
      ...
      procedure Check_9X_Keyword (Token_9X, Next : Token_Type);
      ...
      --  and Token_Type is changed to Token_9X after issuing an appropriate
      --  error message ("the Ada 9X keyword xx cannot be used in Ada 83"). If
      --  not, the scan is backed up with Token_Type unchanged. This routine
      --  is used to deal with an attempt to use a 9X keyword in Ada 83 mode.
      --  The caller has typically checked that the current token, an
      --  identifier, matches one of the 9X keywords.
      ...
      procedure Write_Node_Name (Name_Node : Node_Id);
      --  Writes name associated with given node, which is either a defining
      --  program unit name node, in which case the name is written out with
      --  the required separating dots, or is a node with a Chars field, in
      --  which case the name written is simply this Chars field name.
...
begin
   --  Save old Ada 83 mode, and initialize from compiler switch value

   Save_Ada_83_Mode := Ada_83;
   ...
   Last_Resync_Point := Source'First;

   --  Scan the compilation
   ...
   if Scope.Last /= 0 then
      Compiler_Error;
      Write_Eol;
      Write_Str ("compiler error: scope stack not reset to empty");
      Write_Eol;
      Compiler_Abort;
   end if;
   ...
   --  Restore settings of Ada 83 and Ada 9X mode switches to entry values
