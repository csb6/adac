...
--  initialization routines in Sinput.
   ...
   type Token_Type is (
      ...
      Tok_Integer_Literal, -- numeric lit  Literal

      Tok_Real_Literal,    -- numeric lit  Literal

      Tok_String_Literal,  -- string lit   Literal

      Tok_Char_Literal,    -- char lit     Name, Literal

      Tok_Operator_Symbol, -- op symbol    Name, Literal, Desig

      Tok_Identifier,      -- identifer    Name, Desig
   ...
   Scan_Unit : Unit_Number_Type;
   --  Unit number of source file currently being scanned

   Source : Source_Buffer_Ptr;
   --  Current source (copy of File.Table (Scan_Unit).Source)

   Source_Cache : Source_Buffer_Ptr;
   --  This is used to cache the most recently referenced source buffer to
   --  save lookups in the file table. It must always contain a valid source
   ...
   --  Source file is established to ensure that it always has a valid value.

   Scan_Ptr : Source_Ptr;
   --  Scan pointer. After the Initialize call and after subsequent Next_Line
   --  calls, it points to the first character of the current input line.
   --  After calls to Scan, it is set to point just past the current token.
   ...
   Start_Column : Column_Number_Type;
   ...
   type Saved_Scan_State is record
      Save_Scan_Ptr           : Source_Ptr;         -- save Scan_Ptr
      Save_Token              : Token_Type;         -- save Token
      Save_Token_Ptr          : Source_Ptr;         -- save Token_Ptr
      Save_Current_Line_Start : Source_Ptr;         -- save Current_Line_Start
      Save_Token_Node         : Node_Id;            -- save Token_Node
      Save_Token_Name         : Name_Id;            -- save Token_Name
      Save_Prev_Token         : Token_Type;         -- save Prev_Token
      Save_Prev_Token_Ptr     : Source_Ptr;         -- save Prev_Token_Ptr
      Save_Start_Column       : Column_Number_Type; -- save Start_Column
   end record;
...
--    type Saved_Scan_State is record ...
--    --  To be returned here when private types are working!
