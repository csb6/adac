...
--  information like 'Chars', 'Sloc' or 'Line_Number'.
--  We do this since program trees get lost whenever we start to
--  compile a new unit. Node_Id is only a pointer into program trees.
...
package Xref_Tab is
   ...
   --  An Entity_Table is defined as a twodimensional data list.
   --  The entities and their references appear in the same order as in the
   --  program tree (lower Node_Ids first).
   --  This represents also the order within the Ada programs and is important
   --  if we wanna print the Xref or look for possible overloading effects.
   ...
   --
   --  1) since there is no danger to do so.
   --  2) since it would be very awkward to write all the appropriate read 
   --     and write functions.
   --
   --  Of course, the data structures of entities and references
   --  are full encapsulated - a client can access them only by subprogram
   --  calls.
   ...
   type Kind_Type is (Proc, Func, Genr, Pack, Tsk, Sub, Unknown);
...
-----------------------------   
   ...
   --
   --   Sloc:         Points to the source allocation where the reference
   --                 was found.
   --
   ...
   --
   --   Is_Internal:  Flag, set if information output of the entity should
   --                 be suppressed.
   --
...
--
   ...
   --  If set then warning messages for wrong with clauses appear on the
   --  screen.
   
   Entity_Warnings : Boolean;
   --  If set then warning messages for unused entities appear on the
   --  screen.
   
   Xref_Flag : Boolean;
   --  If set then a cross reference list gets written into a file.
   
   Entity_Info_In_Xref : Boolean;
   --  If set then entities appear with its Entity_Kind information in X.ref.

   Spec_REQs_Flag : Boolean;
   --  If set then the REQs for all withed specs get written into files.
   
   Body_REQs_Flag : Boolean;
   --  If set then the REQs for the body and its subunits get written into
   --  files.
   ...
   --  Maintains the coherence of loaded files and the E_Tables!
   --  If Unit_Number is already in the list only some data gets updated.
   --  Otherwise a new entity table gets added to the end of the entity
   --  table list.
   --  Finally, in every case New_Etbl points to the Entity_Table 
   --  which corresponds to Unit_Number.
   ...
   function Double_Line_Entity  return Boolean;
   --  Checks if the Name_Buffer contains two underlines.
   ...
   --  Used for a withed spec to mark the entities which are used
   --  by its clients.
   --  If the client uses at least one entity then the client entity table
   --  gets also marked to signal a 'dead with clause'.
   ...
   --  Otherwise Add_Entity is called.
   --  Used to sychnonize old Entity_Ids with new Unique_Names.
   --  Finally, in every case New_Entity points to the Entity structure 
   --  which corresponds to Entity_Node.
   ...
   type Output_Level is (Full_Xref, Smart_Xref,
                         Full_Only_Screen, Smart_Only_Screen);
   --  Used only for Writ!
   --
   --  Full:  Empty reference lists cause WARNING messages.
   --  Smart: The output of empty reference lists is supressed.
   --
   --  Xref:        Both screen and file output.
   --  Only_Screen: Only screen output.
   
   procedure Writ (The_Etbl       : Entity_Table_Acc;
                   Level          : Output_Level);
   --  Writes the information stored in the Entity_Table to the Xref Output 
   --  file.
   --  Note: The output of anonymous entities is always supressed.
   ...
   pragma Inline (Double_Line_Entity);
