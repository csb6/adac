...
--  by the front end.
   ...
   type Unit_Record is record
      ...
      Preelab : Boolean;
      --  Indicates presence of PRE parameter

      Elaborate_Body : Boolean;
      --  Indicates presence of EB parameter
   ...
   package Withs is new Table (
      Component_Type => With_Record,
      Index_Type     => With_Id,
      Low_Bound      => First_With_Entry,
      Initial        => 5000,
      Increment      => 200,
      Table_Name     => "With");
   ...
   package Sdep is new Table (
      Component_Type => Sdep_Record,
      Index_Type     => Sdep_Id,
      Low_Bound      => First_Sdep_Entry,
      Initial        => 5000,
      Increment      => 200,
      Table_Name     => "Sdep");
   ...
   package Source is new Table (
      Component_Type => Source_Record,
      Index_Type     => Source_Id,
      Low_Bound      => First_Source_Entry,
      Initial        => 1000,
      Increment      => 200,
      Table_Name     => "Source");
   ...
   --    Source file name    Info field has Source_Id of unit table entry
   ...
   procedure Read_ALI (Afile, Sfile : File_Name_Type);
   --  Reads a specified ALI file and all subsidiary ALI files that it
   --  references. The recursion is terminated by avoiding reading an
   --  ALI file that has already been read. The second parameter is the
   --  name of the corresponding source file (used for error messages only)
   --  If Sfile is No_File (used for main file), then it is a fatal error
