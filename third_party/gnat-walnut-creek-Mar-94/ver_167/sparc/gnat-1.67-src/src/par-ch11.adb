...
package body Ch11 is
   ...
   --  Parsed by P_Identifier_Declaration (3.2)
   ...
   begin
      ...
      if Token = Tok_Exception then
         Set_Exception_Handlers
           (Handled_Stmt_Seq_Node, P_Exception_Handler_Part);
      end if;
   ...
   begin
      ...
      if Token = Tok_Identifier then
         ...
         if Token = Tok_Colon then
            if Ada_83 then
               Error_Msg_SP ("choice parameter not allowed in Ada 83!");
            end if;
            Scan; -- past :
   ...
   ----------------------------------
   -- 11.2  Exception Handler Part --
   ----------------------------------

   --  EXCEPTION_HANDLER_PART ::=
   --    exception EXCEPTION_HANDLER {EXCEPTION_HANDLER}

   --  Note that this is an added rule which does not appear in the official
   --  grammar, which includes it as part of Handled_Sequence_Of_Statements.
   --  The value returned is a list of exception handlers (possibly intermixed
   --  with pragmas).
   ...
   --  The caller has checked that the initial token is EXCEPTION

   --  Error recovery: cannot raise Error_Resync

   function P_Exception_Handler_Part return List_Id is
   ...
   begin
      ...
      if Token = Tok_End then
         ...
         loop
            Append (P_Exception_Handler, Handlers_List);
            P_Pragmas_Opt (Handlers_List);
   ...
   begin
      ...
      if Token = Tok_Others then
         ...
         return P_Qualified_Simple_Name_Resync; -- exception name
      end if;
   ...
   ----------------------------
   -- 11.2  Choice Parameter --
   ----------------------------

   --  Parsed by P_Exception_Handler
   ...
   begin
      ...
      if Token /= Tok_Semicolon then
         Set_Name (Raise_Node, P_Qualified_Simple_Name_Resync);
      end if;
