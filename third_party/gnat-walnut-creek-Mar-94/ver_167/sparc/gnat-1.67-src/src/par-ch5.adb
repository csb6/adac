...
package body Ch5 is
   ...
   function P_Assignment_Statement
                    (LHS             : Node_Id)  return Node_Id;
   function P_Begin_Statement
                    (Block_Name_Node : Node_Id)  return Node_Id;
   ...
   function P_For_Statement
                    (Loop_Name_Node : Node_Id)   return Node_Id;
   ...
   function P_Loop_Statement
                    (Loop_Name_Node : Node_Id)   return Node_Id;
   function P_Null_Statement                     return Node_Id;
   function P_While_Statement
                    (Loop_Name_Node : Node_Id)   return Node_Id;
   ...
   begin
      ...
      loop
         begin
            if GNAT_Style_Check then GNAT_Column_Check (Token_Ptr); end if;
            ...
            then
               if Statement_Required then
                  Error_Msg_BC ("incorrect layout for column check mode");
               end if;
            ...
            if Is_Reserved_Identifier then
               ...
               if
                  ...
                  (Token = Tok_Semicolon and then
                    (Prev_Token /= Tok_Return
            ...
            case Token is
               ...
               when Tok_Exception =>
                  ...
                  then
                     ...
                     Discard_Junk_List (P_Exception_Handler_Part);
                  end if;

                  --  Always return, in the case where we scanned out handlers
                  --  that we did not expect, P_Exception_Handler_Part returned
               ...
               --  Case of THEN, terminate if THEN ABORT allowed (ATC case)

               when Tok_Then =>
                  Save_Scan_State (Scan_State); -- at THEN
                  Scan; -- past THEN

                  if SS_Flags.Tatm and then
                     Token = Tok_Abort
                  then
                     return Statement_List;
                  ...
                  --  it had been there!
                     ...
                     Append (P_If_Statement, Statement_List);
                     Statement_Required := False;
                  end if;
               ...
               when Tok_When | Tok_Others =>
                  ...
                  --  Otherwise complain and skip when Choice {| Choice} =>
                     ...
                     Error_Msg_SC ("WHEN when not allowed here");
                     Scan; -- past when
               ...
               when Tok_Identifier =>
                  ...
                  if Token = Tok_Colon_Equal then
                     Append (P_Assignment_Statement (Id_Node), Statement_List);
                     Statement_Required := False;
                  ...
                  --  we want to speed up as much as possible.
                     ...
                     Append (P_Statement_Call_Name (Id_Node), Statement_List);
                     Scan; -- past semicolon
                     Statement_Required := False;
                  ...
                  then
                     ...
                     Append (P_Goto_Statement, Statement_List);
                     Statement_Required := False;
                  ...
                  then
                     ...
                     if Token = Tok_Loop then
                        Append (P_Loop_Statement (Id_Node), Statement_List);

                     --  While statement (labeled loop statement with WHILE)

                     elsif Token = Tok_While then
                        Append (P_While_Statement (Id_Node), Statement_List);

                     --  Declare statement (labeled block statement with
                     --  DECLARE part)

                     elsif Token = Tok_Declare then
                        Append (P_Declare_Statement (Id_Node), Statement_List);

                     --  Begin statement (labeled block statement with no
                     --  DECLARE part)

                     elsif Token = Tok_Begin then
                        Append (P_Begin_Statement (Id_Node), Statement_List);

                     --  For statement (labeled loop statement with FOR)

                     elsif Token = Tok_For then
                        Append (P_For_Statement (Id_Node), Statement_List);

                     --  Improper statement follows label

                     else
                        Error_Msg_AP
                           ("loop statement or block statement expected");
                     end if;
                  ...
                  --  The only valid possibility is a name extension symbol
                     ...
                     if Expr_Form /= EF_Call then
                        if Token = Tok_Colon_Equal then
                           Append (P_Assignment_Statement (Call_Name_Node),
                                   Statement_List);
                           Statement_Required := False;
                        ...
                        --  Check apostrophe cases
                           ...
                           Append (P_Code_Statement (Call_Name_Node),
                                   Statement_List);
                           Statement_Required := False;
                        ...
                        --  means that the item we just scanned was a call.
                           ...
                           Append (P_Statement_Call_Name (Call_Name_Node),
                             Statement_List);
                           Scan; -- past semicolon
                           Statement_Required := False;
                     ...
                     --  If we did not scan a name, then we must have a call
                     --  or qualified expression (code statement), which means
                     --  that the scan of the statement is complete.

                     else -- Expr_Form = EF_Call
                        TF_Semicolon;
                        Append (P_Statement_Call_Name (Call_Name_Node),
                                                             Statement_List);
                        Statement_Required := False;
                  ...
                  --  as ELSE IF.
                     ...
                     --  If not a bad spelling, then we really have junk
                        ...
                        if Token_Is_At_Start_Of_Line then
                           Append (P_Statement_Call_Name (Id_Node),
                                                           Statement_List);
               ...
               when Tok_Operator_Symbol =>
                  ...
                  if Expr_Form /= EF_Call and then Token = Tok_Colon_Equal then
                     Append (P_Assignment_Statement (Call_Name_Node),
                             Statement_List);
                  else
                     Append (P_Statement_Call_Name (Call_Name_Node),
               ...
               when Tok_Less_Less =>
                  Append (P_Label, Statement_List);
                  Statement_Required := True;

               --  Pragma appearing as a statement in a statement sequence

               when Tok_Pragma =>
                  Append (P_Pragma, Statement_List);

               --  Abort_Statement

               when Tok_Abort =>
                  Append (P_Abort_Statement, Statement_List);
                  Statement_Required := False;

               --  Accept_Statement

               when Tok_Accept =>
                  Append (P_Accept_Statement, Statement_List);
                  Statement_Required := False;

               --  Begin_Statement (Block_Statement with no declare, no label)

               when Tok_Begin =>
                  Append (P_Begin_Statement (Empty), Statement_List);
                  Statement_Required := False;

               --  Case_Statement

               when Tok_Case =>
                  Append (P_Case_Statement, Statement_List);
                  Statement_Required := False;

               --  Block_Statement with DECLARE and no label

               when Tok_Declare =>
                  Append (P_Declare_Statement (Empty), Statement_List);
                  Statement_Required := False;

               --  Delay_Statement

               when Tok_Delay =>
                  Append (P_Delay_Statement, Statement_List);
                  Statement_Required := False;

               --  Exit_Statement

               when Tok_Exit =>
                  Append (P_Exit_Statement, Statement_List);
                  Statement_Required := False;

               --  Loop_Statement with FOR and no label

               when Tok_For =>
                  Append (P_For_Statement (Empty), Statement_List);
                  Statement_Required := False;
               ...
               when Tok_Loop =>
                  Append (P_Loop_Statement (Empty), Statement_List);
                  Statement_Required := False;

               --  Null_Statement

               when Tok_Null =>
                  Append (P_Null_Statement, Statement_List);
                  Statement_Required := False;

               --  Raise_Statement

               when Tok_Raise =>
                  Append (P_Raise_Statement, Statement_List);
                  Statement_Required := False;

               --  Requeue_Statement

               when Tok_Requeue =>
                  Append (P_Requeue_Statement, Statement_List);
                  Statement_Required := False;

               --  Return_Statement

               when Tok_Return =>
                  Append (P_Return_Statement, Statement_List);
                  Statement_Required := False;

               --  Select_Statement

               when Tok_Select =>
                  Append (P_Select_Statement, Statement_List);
                  Statement_Required := False;

               --  While_Statement (Block_Statement with while and no loop)

               when Tok_While =>
                  Append (P_While_Statement (Empty), Statement_List);
                  Statement_Required := False;
   ...
   --  Parsed by P_Sequence_Of_Statements (5.1), except for the case of a
   --  statement of the form of a call or name, which is handled here. The
   --  argument passed in is the tree for the call/name which has been
   --  scanned. The returned value is the corresponding statement form.
   ...
   function P_Statement_Call_Name (Call_Name_Node : Node_Id) return Node_Id is
      Stmt_Node : Node_Id;
   ...
   begin
      ...
      if Nkind (Call_Name_Node) = N_Indexed_Component then
         declare
            Prefix_Node : Node_Id := Prefix (Call_Name_Node);
            Exprs_Node  : List_Id := Expressions (Call_Name_Node);
         begin
            Change_Node (Call_Name_Node, N_Procedure_Call_Statement);
            Set_Name (Call_Name_Node, Prefix_Node);
            Set_Parameter_Associations (Call_Name_Node, Exprs_Node);
            return Call_Name_Node;
         end;

      --  Case of function call, which is a procedure call

      elsif Nkind (Call_Name_Node) = N_Function_Call then
         declare
            Name_Node   : Node_Id := Name (Call_Name_Node);
            Params_List : List_Id := Parameter_Associations (Call_Name_Node);
         begin
            Change_Node (Call_Name_Node, N_Procedure_Call_Statement);
            Set_Name (Call_Name_Node, Name_Node);
            Set_Parameter_Associations (Call_Name_Node, Params_List);
            return Call_Name_Node;
         end;
      ...
      --  All other cases of names are parameterless procedure calls
         ...
         Stmt_Node :=
           New_Node (N_Procedure_Call_Statement, Sloc (Call_Name_Node));
         Set_Name (Stmt_Node, Call_Name_Node);
         return Stmt_Node;
      end if;

   end P_Statement_Call_Name;
   ...
   --  On entry, the caller has scanned the left hand side (passed in as
   --  Lhs), and the scan points to the := (or = in the possible error case)
   ...
   begin
      Assign_Node := New_Node (N_Assignment_Statement, Token_Ptr);
      Scan; -- past colon equal or equal
   ...
   function P_If_Statement return Node_Id is
      ...
      begin
         ...
         Loc := Prev_Token_Ptr;
         Set_Condition (Elsif_Node, P_Expression_No_Right_Paren);
         Check_Then;
         TF_Then;
         Set_Then_Statements
      ...
      begin
         ...
         then
            ...
            Error_Msg_SC ("this token should be@");
         end if;
      end Check_If_Column;

      procedure Check_Then is
      begin
         if Token_Is_At_Start_Of_Line and then Token = Tok_Then then
            Check_If_Column;

            if GNAT_Style_Check
              and then Get_Line_Number (Token_Ptr) = Get_Line_Number (Loc) + 1
            then
   ...
   --  Start of processing for IF statement
   ...
   begin
      ...
      if Token = Tok_If then
         Loc := Token_Ptr;
         Scan; -- past IF
         Set_Condition (If_Node, P_Expression_No_Right_Paren);
      ...
      loop
         if Token = Tok_Elsif then
            ...
            if List_Present (Else_Statements (If_Node)) then
   ...
   --  Condition is parsed by P_Expression (4.4)
   ...
   begin
      ...
      if Is_Empty_List (Alternatives_List) then
         Error_Msg
   ...
   --  This procedure scans only the cases where no iteration scheme is
   --  present. See also P_While_Statement, P_For_Statement.
   ...
   --  is the node identifiers for the block label if any (or is set to Empty
   --  if there is no block label).

   --  Error recovery : can not raise Error_Resync

   function P_Loop_Statement (Loop_Name_Node : Node_Id) return Node_Id is
      Loop_Node : Node_Id;

   begin
      Push_Scope_Stack;
      Scope.Table (Scope.Last).Labl := Loop_Name_Node;
      Scope.Table (Scope.Last).Ecol := Start_Column;
      Scope.Table (Scope.Last).Sloc := Token_Ptr;
      ...
      if No (Loop_Name_Node) then
         Set_Has_Created_Identifier (Loop_Node, True);
         Set_Identifier (Loop_Node,
           Make_Identifier (Sloc (Loop_Node), New_Internal_Name ("loop")));
      else
         Set_Identifier (Loop_Node, Loop_Name_Node);
      end if;
   ...
   --  This procedure scans a loop statement with a FOR iteration scheme
   --  See also P_While_Statement, P_Loop_Statement for other cases.
   ...
   function P_For_Statement (Loop_Name_Node : Node_Id) return Node_Id is
      Loop_Node        : Node_Id;
   ...
   --  See also P_For_Statement, P_Loop_Statement for other cases.
   ...
   function P_While_Statement (Loop_Name_Node : Node_Id) return Node_Id is
      Loop_Node        : Node_Id;
      Iter_Scheme_Node : Node_Id;
   ...
   begin
      ...
      Iter_Scheme_Node := New_Node (N_Iteration_Scheme, Token_Ptr);
      Scan; -- past WHILE
      Set_Condition (Iter_Scheme_Node, P_Expression_No_Right_Paren);
   ...
   --  Parsed by P_For_Statement or P_While_Statement (5.5)
   ...
   --    DEFINING_IDENTIFIER in [reverse] DISCRETE_SUBTYPE_INDICATION
   ...
   ----------------------------
   -- 5.6  Declare Statement --
   ----------------------------

   --  This is a block statement with a DECLARE phrase present
   ...
   --  The caller has checked that the initial token is DECLARE. The parameter
   --  is the node identifier for the block label if any (or is set to Empty
   --  if there is no block label).

   --  Error recovery: cannot raise Error_Resync

   function P_Declare_Statement (Block_Name_Node : Node_Id) return Node_Id is
      Block_Node : Node_Id;
   ...
   begin
      ...
      Scope.Table (Scope.Last).Lreq := Present (Block_Name_Node);
      Scope.Table (Scope.Last).Ecol := Start_Column;
      Scope.Table (Scope.Last).Labl := Block_Name_Node;
      Scope.Table (Scope.Last).Sloc := Token_Ptr;

      Scan; -- past DECLARE

      if No (Block_Name_Node) then
         Set_Has_Created_Identifier (Block_Node, True);
         Set_Identifier (Block_Node,
           Make_Identifier (Sloc (Block_Node), New_Internal_Name ("block")));
      else
         Set_Identifier (Block_Node, Block_Name_Node);
      end if;

      Append_Elmt (Block_Node, Label_List);
      P_Decls_Begin_End (Block_Node);
      return Block_Node;
   ...
   --  This is a block statement with no DECLARE phrase present
   ...
   --  The caller has checked that the initial token is BEGIN. The parameter
   --  is the node identifier for the block label if any (or is set to Empty
   --  if there is no block label).

   --  Error recovery: cannot raise Error_Resync

   function P_Begin_Statement (Block_Name_Node : Node_Id) return Node_Id is
      Block_Node : Node_Id;
   ...
   ------------------------------------------------------
   -- 5.6  Declarations/Begin/End (Also 6.3, 7.1, 9.1) --
   ------------------------------------------------------
   ...
   --    N_Task_Body           task body (begin part optional)
   ...
   --  P_Begin_Statement procedure, which does not call P_Decls_Begin_End.

   --  Error recovery: cannot raise Error_Resync

   procedure P_Decls_Begin_End (Parent : Node_Id) is
      Body_Decl    : Node_Id;
   ...
   begin
         ...
         Outer : while Present (Decl) loop
            ...
            --  items. The inner loop checks the rest of the list.
               ...
               Inner : while Present (Decl) loop
                  if Nkind (Decl) not in N_Later_Decl_Item then
                     Error_Msg_Sloc_1 := Body_Sloc;
                     Error_Msg_N
                       ("in Ada 83 decl cannot appear " &
                        "after body on line #", Decl);
                  end if;
      ...
      --  also reset the expected column for the END to match the BEGIN.
         ...
         --  No BEGIN present
            ...
            --  Otherwise BEGIN is not required for a package body or task
            --  body. However if we have something other than a BEGIN which
            ...
            --  END, EOF, or a token which starts declarations.
                 ...
                 or else Parent_Nkind = N_Task_Body)
              and then (Token = Tok_End
            ...
            --  These are cases in which a BEGIN is required and not present
               ...
               Error_Msg_Sloc_1 := Scope.Table (Scope.Last).Sloc;
               Error_Msg_Node_1 := Scope.Table (Scope.Last).Labl;

               if Parent_Nkind = N_Block_Statement then
                  Error_Msg_BC ("missing BEGIN for DECLARE on line #!");

               elsif Parent_Nkind = N_Entry_Body then
                  Error_Msg_BC ("Missing BEGIN for ENTRY on line #!");

               elsif Parent_Nkind = N_Subprogram_Body then
                  if Nkind (Specification (Parent))
                               = N_Function_Specification
                  then
                     Error_Msg_BC ("missing BEGIN for function& on line #!");
                  else
                     Error_Msg_BC ("missing BEGIN for procedure& on line #!");
                  end if;

               --  The cases for package body and task body arise only when
               --  we have possible statement junk present.

               elsif Parent_Nkind = N_Package_Body then
                  Error_Msg_BC ("missing BEGIN for package body& on line #!");

               elsif Parent_Nkind = N_Task_Body then
                  Error_Msg_BC ("missing BEGIN for task body& on line #!");

               --  No other node type can appear in this position

               else
                  Compiler_Abort; -- should be impossible!
               end if;

               --  Here we pick up the statements after the BEGIN that
               --  should have been present but was not.
      ...
      if Scope.Table (Scope.Last + 1).Etyp = E_Bad_Is then
         Error_Msg ("IS should be semicolon",
           Scope.Table (Scope.Last + 1).S_Is);
         Set_Bad_Is_Detected (Parent, True);
      end if;

   end P_Decls_Begin_End;
   ...
   --  RETURN_STATEMENT has been moved to section 6.5
